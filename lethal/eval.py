"""Head-to-head lethal evaluation: policy A (ship 0) vs policy B (ship 1).

Runs N episodes, then N with sides swapped. Use --a/--b 'random' for a
random-init policy.

Usage:
  uv run python -m lethal.eval --a runs/dog-v1/ckpt_00200.msgpack \\
...
from lethal.env import OBS_DIM, LethalEnv
from lethal.grpo import Policy as ActorCritic
...
def match(a, b, episodes=128, seed=0, reset_mode="fixed"):
    env = LethalEnv(reset_mode=reset_mode)
    network = ActorCritic()

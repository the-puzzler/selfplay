"""Head-to-head stickfight evaluation: policy A (ship 0) vs policy B (ship 1).

Runs N episodes, then N with sides swapped. Use --a/--b 'random' for a
random-init policy.

Usage:
  uv run python -m stickfight.eval --a runs/dog-v1/ckpt_00200.msgpack \\
...
from stickfight.env import OBS_DIM, StickFightEnv
from stickfight.ppo import ActorCritic
...
def match(a, b, episodes=128, seed=0, reset_mode="fixed"):
    env = StickFightEnv(reset_mode=reset_mode)
    network = ActorCritic()
